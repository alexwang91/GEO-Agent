...
> **Technical-preview boundary:** current CI-verifiable behavior is fixture-backed, manual-import oriented, or routed through explicit provider-boundary code. Live crawling requires explicit opt-in and is tested with fake clients in CI. OpenAI-compatible API output is not ChatGPT Search.
...
- **Separates provider truth from provider plans** - implemented, manual, simulated, planned, and unavailable paths use explicit status language.
...
Planned providers must remain planned until deterministic tests and explicit configuration prove the boundary.
...
| CLI | Preview | `geo-agent audit` runs fixture/recorded datasets into an audit package. |
...
- require verified live coverage for ChatGPT Search, Perplexity, Gemini, Google AI Overviews, Claude Search, or Bing Copilot;
...
| Path | Purpose |
|------|---------|
| `src/geo_agent/cli.py` | Fixture-audit CLI entry point |
...
| `src/geo_agent/report_v2.py` | Report sections, metric summaries, and noise-floor delta comparison |
...
| [V8 changelog](docs/v8-changelog.md) | [Verification workflow](.github/workflows/verify.yml) |

## Compared To

| | Scope | Evidence-aware | Provider truth labels | Live-provider claim |
|-|-------|:--------------:|:---------------------:|--------------------|
| **GEO-Agent** | AI visibility audit, diagnosis, task drafts, retest planning | Yes | Yes | Limited to implemented/configured boundaries |
...
- Demo, fixture-backed, simulated, and manual-import artifacts must not be represented as generated live-market audit output.
