...
- **Separates provider truth from provider plans** - implemented, manual, manual-only, simulated, planned, and unavailable paths use explicit status language.
...
# 4. Build a package from explicit manual captures
...
Planned providers must remain planned until deterministic tests and explicit configuration prove the boundary. Manual-only providers must remain manual-only until automated capture is implemented and verified.
...
- require verified automated coverage for ChatGPT Search, Perplexity, Gemini, Google AIO, Claude Search, or Bing Copilot;
